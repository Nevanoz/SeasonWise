...
import { Leaf, Plus, Trash2, Calendar, Coins, ArrowRight, ArrowLeft, Check, HelpCircle } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white border border-border p-6 rounded-xl flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h1 className="text-xl font-bold text-text-primary">Buat Simulasi Arus Kas Baru</h1>
          <p className="text-xs text-text-secondary">Lengkapi data untuk mendeteksi potensi cash gap.</p>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <h2 className="text-base font-bold text-text-primary border-b border-border pb-2">Informasi Rencana & Wilayah</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col gap-2">
                  <label htmlFor="title" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Nama Rencana *</label>
                  <input
                    id="title"
                    type="text"
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="provinceCode" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Provinsi *</label>
                  <select
                    id="provinceCode"
                  ...
                  {errors.provinceCode && <span className="text-xs text-red-600">{errors.provinceCode.message}</span>}
                </div>

                <div className="flex flex-col gap-2">
                  <label htmlFor="regencyCode" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Kabupaten *</label>
                  <select
                    id="regencyCode"
                  ...
                  {errors.regencyCode && <span className="text-xs text-red-600">{errors.regencyCode.message}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <div>
                <h2 className="text-base font-bold text-text-primary">Pilih Template Komoditas</h2>
                <p className="text-xs text-text-secondary mt-0.5">Pilih salah satu untuk memuat asumsi fase tanam & biaya rata-rata.</p>
              </div>
                ...
                {(Object.keys(CROP_TEMPLATES) as CropType[]).map((type) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold">{CROP_TEMPLATES[type].name.split(' ')[0]}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <h2 className="text-base font-bold text-text-primary border-b border-border pb-2">Target & Estimasi Panen</h2>

              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="flex flex-col gap-2">
                  <label htmlFor="plantingDate" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Tanggal Tanam</label>
                  <input
                    id="plantingDate"
                    type="date"
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="expectedHarvestQuantity" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Estimasi Hasil Panen</label>
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="expectedSellingPriceRupiah" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Harga Jual (Rp/Unit)</label>
                  <input
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="expectedTotalHarvestIncomeRupiah" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Total Proyeksi Pendapatan</label>
                  <div className="border border-border p-2.5 rounded-lg text-sm bg-surface-container font-bold text-primary min-h-[44px] flex items-center justify-between">
                    <span>Rp</span>
                    <span>{computedIncome.toLocaleString('id-ID')}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <div className="flex justify-between items-center border-b border-border pb-2">
                <div>
                  <h2 className="text-base font-bold text-text-primary">Biaya Operasional & Pendapatan Tambahan</h2>
                  <p className="text-xs text-text-secondary">Masukkan pengeluaran masa tanam hingga panen.</p>
                </div>
                <button
                  type="button"
                  onClick={() => append({
                  ...
                  return (
                    <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 border border-border rounded-lg relative bg-background/10">
                      <div className="md:col-span-2">
                        <label htmlFor={`cf-type-${idx}`} className="text-[10px] font-bold uppercase text-text-secondary">Jenis</label>
                        <select
                          id={`cf-type-${idx}`}
                          disabled={isHarvest}
                          {...register(`cashFlowItems.${idx}.type` as const)}
                          className="w-full border border-border p-2 rounded text-xs bg-white min-h-[38px] disabled:opacity-50"
                        >
                      ...
                      <div className="md:col-span-3">
                        <label htmlFor={`cf-category-${idx}`} className="text-[10px] font-bold uppercase text-text-secondary">Kategori</label>
                        <input
                          id={`cf-category-${idx}`}
                          type="text"
                          disabled={isHarvest}
                          ...
                          className="w-full border border-border p-2 rounded text-xs bg-white min-h-[38px]"
                        />
                      </div>

                      <div className="md:col-span-3">
                        <label htmlFor={`cf-amount-${idx}`} className="text-[10px] font-bold uppercase text-text-secondary">Jumlah (Rupiah)</label>
                        <input
                          id={`cf-amount-${idx}`}
                          type="number"
                          placeholder="0"
                      ...
                      <div className="md:col-span-3">
                        <label htmlFor={`cf-date-${idx}`} className="text-[10px] font-bold uppercase text-text-secondary">Tanggal</label>
                        <input
                          id={`cf-date-${idx}`}
                          type="date"
            ...
            {/* Domestik & Saldo Awal */}
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <h2 className="text-base font-bold text-text-primary border-b border-border pb-2">Kebutuhan Rumah Tangga & Saldo Awal</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col gap-2">
                  <label htmlFor="monthlyHouseholdExpenseRupiah" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Pengeluaran Keluarga / Bulan</label>
                  <input
                    ...
                    className="border border-border p-2.5 rounded-lg text-sm bg-background/30 min-h-[44px]"
                  />
                  {errors.monthlyHouseholdExpenseRupiah && <span className="text-xs text-red-600">{errors.monthlyHouseholdExpenseRupiah.message}</span>}
                </div>

                <div className="flex flex-col gap-2">
                  <label htmlFor="openingBalanceRupiah" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Saldo Awal Kas Aktif</label>
                  <input
                ...
                <div className="flex flex-col gap-2">
                  <label htmlFor="emergencyReserveRupiah" className="text-xs font-bold uppercase tracking-wider text-text-secondary">Simpanan / Dana Darurat</label>
                  <input
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white font-bold rounded-lg disabled:opacity-50 hover:opacity-90 min-h-[44px]"
              >
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <div className="flex justify-between items-center border-b border-border pb-2">
                <div>
                  <h2 className="text-base font-bold text-text-primary">Struktur Pembiayaan (Pinjaman)</h2>
                  <p className="text-xs text-text-secondary">Masukkan pengajuan pinjaman untuk mensimulasikan beban cicilan.</p>
                </div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="border border-border p-4 rounded-xl space-y-4 bg-background/5">
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                      <label htmlFor="primary-loan-name" className="text-[10px] font-bold uppercase text-text-secondary">Nama Kreditur / Program</label>
                      <input
                        id="primary-loan-name"
                        type="text"
                        {...register('financingOptions.0.name')}
                        className="border border-border p-2 rounded text-xs bg-white min-h-[38px]"
                      />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-principal" className="text-[10px] font-bold uppercase text-text-secondary">Pokok Plafon (Rp)</label>
                        <input
                      ...
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-rate" className="text-[10px] font-bold uppercase text-text-secondary">Suku Bunga (%)</label>
                        <div className="flex rounded border border-border overflow-hidden bg-white">
                          ...
                          />
                          <span className="p-2 text-xs text-text-secondary bg-[#E8EFE8] font-bold">%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-rate-period" className="text-[10px] font-bold uppercase text-text-secondary">Metode Bunga</label>
                        <select
                      ...
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-repay-structure" className="text-[10px] font-bold uppercase text-text-secondary">Struktur Repayment</label>
                        <select
                          ...
                          className="border border-border p-2 rounded text-xs bg-white min-h-[38px]"
                        >
                          <option value="FLAT_MONTHLY">Cicilan Bulanan Flat</option>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-installments" className="text-[10px] font-bold uppercase text-text-secondary">Jumlah Cicilan</label>
                        <input
                          ...
                          className="border border-border p-2 rounded text-xs bg-white min-h-[38px] disabled:opacity-50"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-admin-fee" className="text-[10px] font-bold uppercase text-text-secondary">Biaya Admin (Upfront)</label>
                        <input
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-disburse-date" className="text-[10px] font-bold uppercase text-text-secondary">Tanggal Pencairan</label>
                        <input
                      ...
                      <div className="flex flex-col gap-1">
                        <label htmlFor="primary-repay-date" className="text-[10px] font-bold uppercase text-text-secondary">Tanggal Jatuh Tempo I</label>
                        <input
                ...
                {compareEnabled ? (
                  <div className="border-2 border-dashed border-primary/30 p-4 rounded-xl space-y-4 bg-primary/5">
                    <h3 className="text-sm font-bold text-[#986924] flex items-center gap-1.5">
                      <Coins className="w-4 h-4" /> Skema Pembanding (B)
                    </h3>

                    <div className="space-y-3">
                      <div className="flex flex-col gap-1">
                        <label htmlFor="secondary-loan-name" className="text-[10px] font-bold uppercase text-text-secondary">Nama Kreditur / Program</label>
                        <input
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-principal" className="text-[10px] font-bold uppercase text-text-secondary">Pokok Plafon (Rp)</label>
                          <input
                        ...
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-rate" className="text-[10px] font-bold uppercase text-text-secondary">Suku Bunga (%)</label>
                          <div className="flex rounded border border-border overflow-hidden bg-white">
                            <input
                              id="secondary-rate"
                              type="number"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-rate-period" className="text-[10px] font-bold uppercase text-text-secondary">Metode Bunga</label>
                          <select
                        ...
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-repay-structure" className="text-[10px] font-bold uppercase text-text-secondary">Struktur Repayment</label>
                          <select
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-installments" className="text-[10px] font-bold uppercase text-text-secondary">Jumlah Cicilan</label>
                          <input
                        ...
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-admin-fee" className="text-[10px] font-bold uppercase text-text-secondary">Biaya Admin (Upfront)</label>
                          <input
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-disburse-date" className="text-[10px] font-bold uppercase text-text-secondary">Tanggal Pencairan</label>
                          <input
                        ...
                        <div className="flex flex-col gap-1">
                          <label htmlFor="secondary-repay-date" className="text-[10px] font-bold uppercase text-text-secondary">Tanggal Jatuh Tempo I</label>
                          <input
                  ...
                  <div className="border border-dashed border-border p-6 rounded-xl flex flex-col justify-center items-center text-center text-text-secondary space-y-3 min-h-[250px]">
                    <HelpCircle className="w-8 h-8 opacity-40 text-text-secondary" />
                    <p className="text-xs">Bandingkan skema cicilan musiman (Bullet) vs flat bulanan untuk meminimalkan risiko cash gap sebelum panen.</p>
                    <button
                      type="button"
                      onClick={handleToggleCompare}
                      className="px-4 py-2 text-xs bg-white hover:bg-background/20 text-primary border border-primary rounded-lg font-bold min-h-[44px]"
                    >
                      Aktifkan Pembanding
                    </button>
                  </div>
            ...
            <div className="flex justify-between pt-4">
              <button
                type="button"
                onClick={prevStep}
                className="inline-flex items-center gap-2 px-6 py-3 border border-border text-text-secondary bg-white font-bold rounded-lg hover:bg-background/20 min-h-[44px]"
              >
              ...
              <button
                type="button"
                onClick={nextStep}
                className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white font-bold rounded-lg hover:opacity-90 min-h-[44px]"
              >
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="bg-white border border-border p-6 rounded-xl space-y-4">
              <h2 className="text-base font-bold text-text-primary border-b border-border pb-2">Tinjau Data Rencana</h2>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                <div className="space-y-3">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-text-secondary">Detail Lahan & Komoditas</h3>
                  <div className="space-y-1">
                    <p className="flex justify-between"><span>Judul Rencana:</span> <span className="font-bold">{watch('title')}</span></p>
                    <p className="flex justify-between"><span>Komoditas:</span> <span className="font-bold">{watchCropPlan?.cropType?.toUpperCase()}</span></p>
                    <p className="flex justify-between"><span>Lokasi:</span> <span className="font-bold">ID-{watch('provinceCode')}-{watch('regencyCode')}</span></p>
                    <p className="flex justify-between"><span>Mulai Tanam:</span> <span className="font-bold">{watchCropPlan?.plantingDate}</span></p>
                    <p className="flex justify-between"><span>Estimasi Hasil:</span> <span className="font-bold">{watchCropPlan?.expectedHarvestQuantity} {watchCropPlan?.quantityUnit}</span></p>
                    <p className="flex justify-between"><span>Total Proyeksi Pendapatan:</span> <span className="font-bold text-primary">Rp {computedIncome.toLocaleString('id-ID')}</span></p>
                  </div>
                </div>

                <div className="space-y-3">
                  <h3 className="font-bold text-xs uppercase tracking-wider text-text-secondary">Keuangan Rumah Tangga & Kas</h3>
                  <div className="space-y-1">
                    <p className="flex justify-between"><span>Sewa & Upah Kerja Lahan:</span> <span className="font-bold">{fields.filter(x => x.type === 'production_expense').length} Transaksi</span></p>
                    <p className="flex justify-between"><span>Dana Rumah Tangga / Bln:</span> <span className="font-bold">Rp {parseInt(watch('monthlyHouseholdExpenseRupiah')?.toString() || '0').toLocaleString('id-ID')}</span></p>
                    <p className="flex justify-between"><span>Saldo Awal Kas:</span> <span className="font-bold text-primary">Rp {parseInt(watch('openingBalanceRupiah')?.toString() || '0').toLocaleString('id-ID')}</span></p>
                    <p className="flex justify-between"><span>Dana Cadangan / Darurat:</span> <span className="font-bold">Rp {parseInt(watch('emergencyReserveRupiah')?.toString() || '0').toLocaleString('id-ID')}</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-border pt-4 space-y-4">
                <h3 className="font-bold text-xs uppercase tracking-wider text-text-secondary">Struktur Pembiayaan</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                  {watchFinancingOptions.map((opt, idx) => (
                    <div key={opt.id} className="bg-background/25 border border-border p-4 rounded-xl">
                      <p className="font-bold text-xs text-primary uppercase mb-2">Skema Pembiayaan {idx === 0 ? 'Utama (A)' : 'Pembanding (B)'}</p>
                      <p className="flex justify-between"><span>Kreditur:</span> <span className="font-bold">{opt.name}</span></p>
                      <p className="flex justify-between"><span>Plafon Pokok:</span> <span className="font-bold">Rp {parseInt(opt.principalRupiah?.toString() || '0', 10).toLocaleString('id-ID')}</span></p>
                      <p className="flex justify-between"><span>Suku Bunga:</span> <span className="font-bold">{(opt.interestRateBps / 100).toFixed(2)}% per {opt.interestPeriod === 'MONTHLY' ? 'Bulan' : 'Tahun'}</span></p>
                      <p className="flex justify-between"><span>Bentuk Cicilan:</span> <span className="font-bold">{opt.repaymentStructure === 'FLAT_MONTHLY' ? 'Flat Bulanan' : 'Pascapanen (Bullet)'}</span></p>
                      <p className="flex justify-between"><span>Tenor Pembayaran:</span> <span className="font-bold">{opt.numberOfInstallments} Kali</span></p>
                    </div>
                  ))}
                </div>
              </div>
